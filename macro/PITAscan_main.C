...
void PITAscan_main(int run_num = 3646, TString mainBCM = "bcm_dg_ds", TString ucut = "cleandata&&scandata2<10&&!(CodaEventNumber>48800&&CodaEventNumber<50000)&&!(CodaEventNumber>53000&&CodaEventNumber<54000)"){
//void IAScan_DET(){
  ...
  TString device_name[] =
    ...
    //   "asym_sam5","asym_sam6","asym_sam7","asym_sam8"};
     {"asym_usl","asym_dsl","asym_usr","asym_dsr","asym_usl+asym_bcm_dg_ds","asym_dsl+asym_bcm_dg_ds","asym_usr+asym_bcm_dg_ds","asym_dsr+asym_bcm_dg_ds"};

  // TString device_name2[] =
  //{"asym_atl1","asym_atl2","asym_atr1","asym_atr2","cor_atl1","cor_atl2","cor_atr1","cor_atr2"};
  ...
  TCanvas *c_res = new TCanvas("c_res","c_res",150,150,1700,1000);
 ...
 tree->Draw(Form("(asym_%s/ppm-%6.5f*scandata1-%6.5f):scandata1>>mybcmres",mainBCM.Data(),myslope,myoffs),my_cut.Data(),"prof");
  ...
  h_bcmres[ibcm]->SetTitle(Form("%s Residual",yTitle.Data()));
   ...
   gSystem->Exec(Form("pdfunite %s/PITAscan_run%d*.pdf %s/run%d_PITAscanall.pdf",mydir.Data(),run_num,mydir.Data(),run_num));
    gSystem->Exec(Form("rm -rf PITA*.pdf"));
