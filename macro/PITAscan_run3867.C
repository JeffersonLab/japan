...
void PITAscan_main_run3867(int run_num = 3867, TString mainBCM = "bcm_an_ds", TString ucut = "1"){
//void IAScan_DET(){
 ...
 tree->Draw(Form("(asym_%s/ppm-%6.5f*scandata1-%6.5f):scandata1>>mybcmres",mainBCM.Data(),myslope,myoffs),my_cut.Data(),"prof");
  ...
  h_bcmres[ibcm]->SetTitle(Form("%s Residual",yTitle.Data()));
   ...
   gSystem->Exec(Form("pdfunite %s/PITAscan_run%d*.pdf %s/run%d_PITAscan.pdf",mydir.Data(),run_num,mydir.Data(),run_num));
