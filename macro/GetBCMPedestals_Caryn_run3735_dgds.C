...
void GetBCMPedestals_Caryn_run3735_dgds(int run_num = 3735,TString user_cut ="1",TString scan_data = "(bcm_dg_ds.hw_sum_raw/bcm_dg_ds.num_samples--396.60)*0.002328",double MIN=5,double MAX=75,Int_t myii=0){
...
//"sam1","sam2","sam3","sam4","sam5","sam6","sam7","sam8",
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_bcm_pedestal_fit_dgds.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
