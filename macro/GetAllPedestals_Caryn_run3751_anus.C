...
void GetAllPedestals_Caryn_run3751_anus(int run_num = 3751,TString user_cut ="!(scandata1==700&&CodaEventNumber>85800&&CodaEventNumber<89300)&&!(scandata1==550&&CodaEventNumber>121e3&&CodaEventNumber<126e3)&&!(scandata1==300&&CodaEventNumber>162e3)",TString scan_data ="(bcm_an_us.hw_sum_raw/bcm_an_us.num_samples--696.84)*0.006026" ,double MIN=35,double MAX=75,Int_t myii=0){

  //"(bcm_an_ds.hw_sum_raw/bcm_an_ds.num_samples--537.44)*0.006133
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  double gain[nDET];
	...
	sprintf(outfilename,"%s/run%d_detsamcavbcm_pedestal_fit_pedsearch1070_ands.txt",
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_det_pedestal_fit_anus_40to70.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
