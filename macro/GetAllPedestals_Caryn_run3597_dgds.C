...
void GetAllPedestals_Caryn_run3597_dgds(int run_num = 3597,TString user_cut ="1",TString scan_data = "(bcm_dg_ds.hw_sum_raw/bcm_dg_ds.num_samples- -210.26)*0.002717",double MIN=55,double MAX=75,Int_t myii=0){


  user_cut = Form("%s&&bmwobj==-1&&!(cleandata&&scandata1==400&&bcm_an_ds<37)",user_cut.Data());
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  double gain[nDET];
	...
	sprintf(outfilename,"%s/run%d_detsamcavbcm_pedestal_fit_5570dgds.txt",
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_det_pedestal_fit_5570dgds.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
