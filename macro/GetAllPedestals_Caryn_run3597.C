...
void GetAllPedestals_Caryn_run3597(int run_num = 3597,TString user_cut ="1",TString scan_data = "(bcm_an_ds.hw_sum_raw/bcm_an_ds.num_samples--387.81)*0.002774",double MIN=55,double MAX=75,Int_t myii=0){
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  double gain[nDET];
	...
	sprintf(outfilename,"%s/run%d_detsamcavbcm_pedestal_fit_5070.txt",
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_det_pedestal_fit_5070.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
