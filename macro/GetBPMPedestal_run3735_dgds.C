...
void GetBPMPedestal_run3735_dgds(int run_num=3735, 
		    TString bcm_name="(bcm_dg_ds.hw_sum_raw/bcm_dg_ds.num_samples--396.60)*0.002328",
  ...
  TCut beam_evtcut[] ={ 
    ...
    "cleandata && scandata1==448 && scandata2==3",
    "cleandata && scandata1==398 && scandata2==3",
    "cleandata && scandata1==607 && scandata2==3"};
  ...
  for(int ibpm=0;ibpm<nBPM;ibpm++){
    ...
    c1->SaveAs(Form("run%d_bpm%s_ped_fit.png",run_num,device_name[ibpm].Data()));
  } // End of BPMs loop
  gSystem->Exec(Form("convert $(ls -rt *bpm*fit.png) run%d_bpm_pedestal_fit.pdf",run_num));
  gSystem->Exec("rm *bpm*.png");
