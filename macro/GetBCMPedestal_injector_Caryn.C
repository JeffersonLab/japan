...
void GetBCMPedestal_injector_Caryn(int run_num = 1, TString scan_data = "scandata1",TString user_cut ="1",Double_t MIN=-1e6, Double_t MAX=1e6, Int_t myii=0){
  ...
  c2->Divide(1,2);
  TVirtualPad* c_fit = c2->cd(1);
  TVirtualPad* c_res = c2->cd(2);
  c_fit->Divide(1,1);
  c_res->Divide(1,1);
...
char outfilename[255];
  ...
  for(int ibpm=0;ibpm<nBPM;ibpm++){
	...
	mg_fit[ich]->Add(g_fit_ref[ich]);
      ...
      gain[ibpm][ich] = 1/slope[ibpm][ich];
    ...
    c2->SaveAs(Form("%s/run%d_bcm%s_ped_fit.png",outputDir.Data(),run_num,device_name[ibpm].Data()));
  } // End of BPMs loop
  gSystem->Exec(Form("convert %s/*bcm*fit.png %s/run%d_injector_bcm_pedestal_fit.pdf",outputDir.Data(),outputDir.Data(),run_num));
  gSystem->Exec(Form("rm  %s/run%d_bcm*fit.png",outputDir.Data(),run_num));
  ...
  delete rootfile, tree, outputDir, htemp, max, nbinx, vec_scandata, hsd, bin_content, bin_center, ndata, ndata_fit, scandata, scandata_error, startpt, device_name, nBPM,  f_zero, branch_name, num_samples_name, my_cut, adc_mean, adc_error, adc_res, gfit_title, gres_title, ped, slope, outfilename, outfile, f_fit, init_par;
  //delete rf_name
  //delete h_stat;
