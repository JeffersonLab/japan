...
void PITAscan_main_run4900(int run_num = 4900, TString mainBCM = "bcm_an_us", TString ucut = "ErrorFlag==0&&cleandata"){

  //type = "dets" or "sams" or "ats"
...
TString device_name[] =
  ...
  TString bcm_name[]=
    {"asym_bcm_an_us","asym_bcm_an_ds","asym_bcm_dg_us","asym_cav4cQ","asym_bcm_dg_ds"};
 ...
 tree->Draw(Form("(asym_%s/ppm-%6.5f*scandata1-%6.5f):scandata1>>mybcmres",mainBCM.Data(),myslope,myoffs),my_cut.Data(),"prof");
  ...
  h_bcmres[ibcm]->SetTitle(Form("%s Residual",yTitle.Data()));
   ...
   gSystem->Exec(Form("pdfunite %s/PITAscan_run%d*.pdf %s/run%d_PITAscan_%s.pdf",mydir.Data(),run_num,mydir.Data(),run_num,mainBCM.Data()));
   gSystem->Exec(Form("rm -rf %s/PITA*run%d*.pdf",mydir.Data(),run_num));

    cout<<Form("asym_%s pita-slope ppm/count: ",mainBCM.Data())<<myslope<<endl;
