...
void PITAScan_BPM(int run_num = 3431){
//void IAScan_DET(){
  ...
  TString device_name[] =
    ...
    {"diff_bpm12X","diff_bpm11X","diff_bpm4aX","diff_bpm4aY","diff_bpm4eX","diff_bpm4eY"};
  TString bcm_name[]=
  ...
  TCanvas *c_sam_det = new TCanvas("c_sam_det","c_sam_det",1200,600);
  ...
  for(int idet=0;idet<nDET;idet++){
      ...
      branch_name = Form("(%s)/um",device_name[idet].Data());
...
TCanvas *c_bcm = new TCanvas("c_bcm","c_bcm",1200,600);
  ...
  c_bcm->cd(1);
   ...
   for(int ibcm=0;ibcm<nBCM-1;ibcm++){
    ...
    gSystem->Exec(Form("pdfunite *.pdf ~/PREX/japan/plots/parityOutputs/run%d_PITAscan_bpm.pdf",run_num));
