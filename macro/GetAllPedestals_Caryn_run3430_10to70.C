...
void GetAllPedestals_Caryn_run3430_10to70(int run_num = 3430,TString user_cut ="1",TString scan_data = "(bcm_an_ds.hw_sum_raw/bcm_an_ds.num_samples--387.81)*0.002774",double MIN=5,double MAX=75,Int_t myii=0){
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_det_pedestal_fit_pedsearch1070.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
