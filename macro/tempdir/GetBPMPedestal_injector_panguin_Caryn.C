...
const int nBPM= sizeof(device_name)/sizeof(*device_name);
  ...
  for(int ibpm=0;ibpm<nBPM;ibpm++){
	...
	mg_fit[ich]->Add(g_fit_ref[ich]);
      mg_fit[ich]->Draw("AP");
      gfit_title[ich] = Form("bpm%s%s;beam_current; Raw ADC",device_name[ibpm].Data(),ch_name[ich].Data());
	...
	mg_res[ich]->Add(g_res_ref[ich]); 
      mg_res[ich]->Draw("AP");
      gres_title[ich] = Form("bpm%s%s;beam_current; Residual ",device_name[ibpm].Data(),ch_name[ich].Data());
