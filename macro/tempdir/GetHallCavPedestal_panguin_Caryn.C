...
void GetHallCavPedestal_panguin_Caryn(TString user_cut ="1", double MIN = -1000, double MAX = 1000,Int_t myii=0){
  ...
  for(int i=0;i<ndata;i++){
    ...
    tree->Draw(Form("scandata1>>h_unser_%d_%d",i,myii),
	       my_cut.Data(),"goff");
    ...
    tree->Draw(Form("scandata1>>h_ped_%d_%d",i,myii),
	       my_cut.Data(),"goff");
  ...
  for(int ibcm=0;ibcm<nbcm;ibcm++){
	...
	bcm_error[i] = h_stat->GetRMS()/TMath::Sqrt(h_stat->GetEntries());
      ...
      g_bcmc_fit = new TGraphErrors(ndata,unser_mean,bcm_mean,unser_error,bcm_error);
      g_bcmc_fit->SetMarkerStyle(20);
      g_bcmc_fit->Draw("AP");
      gfit_title = Form("%s; UNSER(uA); Raw ADC per Sample",device_name[ibcm].Data());
      g_bcmc_fit->SetTitle(gfit_title.Data());
      f_fit->SetParameters(init_par);
      f_fit->SetLineColor(kRed);
      g_bcmc_fit->Fit("f_fit","QR","",MIN,MAX);
	...
	bcm_error[i] = bcm_error[i] *gain[ibcm];
      ...
      g_bcmc_res->Draw("AP");
      gres_title = Form("%s;UNSER(uA); Residual(uA) ",device_name[ibcm].Data());
      g_bcmc_res->SetTitle(gres_title.Data());
      f_zero->Draw("same");
