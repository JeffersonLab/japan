...
void GetAllPedestals_Caryn_run4610_dgds(int run_num = 4610,TString user_cut ="cleandata&&!(scandata1==700&&bcm_an_us<65)&&!(scandata1>300&&bcm_an_us<35)",TString scan_data ="(bcm_dg_ds.hw_sum_raw/bcm_dg_ds.num_samples--351.06)*0.002318" ,double MIN=45,double MAX=75,Int_t myii=0){
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_det_pedestal_fit_dgds_50to70.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
