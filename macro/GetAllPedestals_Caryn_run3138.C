...
void GetAllPedestals_Caryn_run3138(int run_num = 3138,TString user_cut ="1",TString scan_data = "bcm_an_ds3",double MIN=37,double MAX=70,Int_t myii=0){
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_det_pedestal_fit.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
