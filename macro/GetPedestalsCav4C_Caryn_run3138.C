...
void GetPedestalsCav4C_Caryn_run3138(int run_num = 3138,TString user_cut ="1",TString scan_data = "bcm_an_ds3",double MIN=37,double MAX=70,Int_t myii=0){
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  double gain[nDET];
	...
	sprintf(outfilename,"%s/run%d_cav_pedestal_fit.txt",
  ...
  for(int idet=0;idet<nDET;idet++){
	...
	det_error[i] = h_stat->GetRMS()/TMath::Sqrt(h_stat->GetEntries());
      ...
      g_fit = new TGraphErrors(6,unser_mean,det_mean,unser_error,det_error);
	...
	det_res[i] = det_mean[i] - f_fit->Eval(unser_mean[i]);
      ...
      g_res = new TGraphErrors(6,unser_mean,det_res,unser_error,det_error);
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_cav_pedestal_fit.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
