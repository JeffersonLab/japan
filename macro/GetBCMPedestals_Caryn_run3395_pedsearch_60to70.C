...
void GetBCMPedestals_Caryn_run3395_pedsearch_60to70(int run_num = 3395,TString user_cut ="1",TString scan_data = "(bcm_an_ds.hw_sum_raw/bcm_an_ds.num_samples+340.4)*0.002781",double MIN=57,double MAX=75,Int_t myii=0){
...
//"sam1","sam2","sam3","sam4","sam5","sam6","sam7","sam8",
  ...
  double gain[nDET];
	...
	sprintf(outfilename,"%s/run%d_detsamcavbcm_pedestal_fit_fittods1x60_70.txt",
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_bcm_pedestal_fit_fittods1x60_70.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
