...
void GetAllPedestals_Caryn_run3645_dgds(int run_num = 3645,TString user_cut ="!(scandata2==4&&scandata1>0&&unser<40)&&!(scandata2==2&&scandata1>0&&unser<60)&&!(scandata2==6&&unser<20)",TString scan_data ="(bcm_dg_ds.hw_sum_raw/bcm_dg_ds.num_samples--500.22)*0.002338" ,double MIN=5,double MAX=75,Int_t myii=0){
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  double gain[nDET];
	...
	sprintf(outfilename,"%s/run%d_detsamcavbcm_pedestal_fit_pedsearch6070.txt",
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_det_pedestal_fit_10to70.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
