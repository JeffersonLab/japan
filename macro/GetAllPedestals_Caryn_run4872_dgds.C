...
void GetAllPedestals_Caryn_run4872_dgds(int run_num = 4872,TString user_cut ="cleandata&&!(scandata1>600&&ErrorFlag!=0)&&!(scandata2==2&&bcm_an_us<75)&&!(scandata2==3&&CodaEventNumber<265e3)&&!(scandata2==8&&bcm_an_us<25)&&!(scandata2==10&&CodaEventNumber<492.5e3)&&CodaEventNumber<492e3&&!(CodaEventNumber>86e3&&CodaEventNumber<90e3)",TString scan_data ="(bcm_dg_ds.hw_sum_raw/bcm_dg_ds.num_samples--493.75)*0.002346" ,double MIN=5,double MAX=100,Int_t myii=0){
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  double gain[nDET];
	...
	sprintf(outfilename,"%s/run%d_detsamcavbcm_pedestal_fit_pedsearch15to85.txt",
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_det_pedestal_fit_dgds_15to85.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
