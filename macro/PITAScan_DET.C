...
void PITAScan_DET(int run_num = 3431){
//void IAScan_DET(){
  ...
  TString device_name[] =
    ...
    {"asym_usl","asym_dsl","asym_usr","asym_dsr","cor_usl","cor_dsl","cor_usr","cor_dsr"};
  ...
  TCanvas *c_sam_det = new TCanvas("c_sam_det","c_sam_det",1700,1000);
...
TCanvas *c_bcm = new TCanvas("c_bcm","c_bcm",1200,600);
  ...
  c_bcm->cd(1);
   ...
   for(int ibcm=0;ibcm<nBCM-1;ibcm++){
    ...
    gSystem->Exec(Form("pdfunite *.pdf ~/PREX/japan/plots/parityOutputs/run%d_PITAscan_det.pdf",run_num));
