...
void GetBCMPedestals_Caryn_run3751_anus(int run_num = 3751,TString user_cut ="!(scandata1==700&&CodaEventNumber>85800&&CodaEventNumber<89300)&&!(scandata1==550&&CodaEventNumber>121e3&&CodaEventNumber<126e3)&&!(scandata1==300&&CodaEventNumber>162e3)",TString scan_data = "(bcm_an_us.hw_sum_raw/bcm_an_us.num_samples--696.84)*0.006026",double MIN=45,double MAX=75,Int_t myii=0){
  //"(bcm_an_ds.hw_sum_raw/bcm_an_ds.num_samples--387.81)*0.002774"
  //-340.4, 0.002781"(bcm_an_ds.hw_sum_raw/bcm_an_ds.num_samples+340.4)*0.002781
  //"(bcm_an_ds.hw_sum_raw/bcm_an_ds.num_samples+340.4)*0.002781"
  //  user_cut = Form("%s&&scandata2!=0",user_cut.Data());
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  TString device_name[] =
    {"bcm_an_us","bcm_an_ds3","bcm_an_ds","bcm_dg_us","bcm_dg_ds","bcm_an_ds10","cav4cQ"};
//"sam1","sam2","sam3","sam4","sam5","sam6","sam7","sam8",
  ...
  double gain[nDET];
	...
	sprintf(outfilename,"%s/run%d_bcm_pedestal_fit_ands.txt",
  ...
  for(int idet=0;idet<nDET;idet++){
	...
	det_error[i] = h_stat->GetRMS()/TMath::Sqrt(h_stat->GetEntries());
      ...
      //  else
      // if(branch_name.Contains("ds3"))
	//	 g_fit->Fit("f_fit","QR","",57,75);
      // else
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_bcm_pedestal_fit_anus_50to70.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
