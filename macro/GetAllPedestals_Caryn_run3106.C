...
void GetAllPedestals_Caryn_run3106(int run_num = 3106,TString user_cut ="1",TString scan_data = "bcm_an_us",double MIN=35,double MAX=55,Int_t myii=0){


    user_cut = Form("%s&&bcm_an_us>15",user_cut.Data());
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  int myscanN[5]={495,453,399,302,203};

  TString my_cut;
  for(int i=0;i<ndata;i++){
    cout<<myscanN[i]<<endl;
    my_cut = Form("cleandata==1&&scandata1==%d&&scandata1!=0&&%s",myscanN[i],user_cut.Data());
  ...
  for(int idet=0;idet<nDET;idet++){
    branch_name = Form("%s.hw_sum_raw/%s.num_samples",
      ...
      for(int i=0;i<ndata;i++){
	my_cut = Form("cleandata==1&&scandata2==%d&&scandata1!=0&&%s",myscanN[i],user_cut.Data());
  ...
  gSystem->Exec(Form("convert %s/*fit.png %s/run%d_det_pedestal_fit.pdf",outputDir.Data(),outputDir.Data(),run_num));
