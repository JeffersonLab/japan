...
void GetHallBCMPedestal_run3396(int run_num=3396 ,TString user_cut ="1",TString scan_data= "unser", double MIN = -1000, double MAX = 1000,Int_t myii=0){
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  tree->Draw(Form("scandata1>>htemp%d",myii),user_cut.Data(),"goff");
  ...
  tree->Draw(Form("scandata1>>hsd%d",myii),Form("%s&&scandata1!=0 && cleandata",user_cut.Data()),"goff");
  int bin_content; 
  double bin_center;
 ...
 double slope[nbcm];
  ...
  for(int ibcm=0;ibcm<nbcm;ibcm++){
    branch_name = Form("%s.hw_sum_raw/%s.num_samples",
      ...
      for(int i=0;i<ndata;i++){
	my_cut = Form("cleandata==1 &&scandata1==%d",(int)vec_scandata[i]);
	...
	bcm_error[i] = h_stat->GetRMS()/TMath::Sqrt(h_stat->GetEntries());
      ...
      g_fit->Draw("AP");
      gfit_title = Form("%s; %s; Raw ADC per Sample",branch_name.Data(),scan_data.Data());
      g_fit->SetTitle(gfit_title.Data());
      f_fit->SetParameters(init_par);
      f_fit->SetLineColor(kRed);
      if(device_name[ibcm]=="bcm_an_ds10")
	{      g_fit->Fit("f_fit","Q","",19,37);}
      else{
	...
	bcm_error[i] = bcm_error[i];
      ...
      g_res->Draw("AP");
      gres_title = Form("%s;%s; Residual(uA) ",branch_name.Data(),scan_data.Data());
      g_res->SetTitle(gres_title.Data());
      f_zero->Draw("same");

      c_bcm->SaveAs(Form("%s/run%d_%s_ped_fit.png",outputDir.Data(),run_num,device_name[ibcm].Data()));
  } // End of BCMs loop
  gSystem->Exec(Form("convert %s/*bcm*fit.png %s/run%d_bcm_pedestal_fit.pdf",outputDir.Data(),outputDir.Data(),run_num));
  ...
  delete rootfile, tree, outputDir, device_name, nbcm,f_zero, branch_name, num_samples_name, ndata, bcm_mean, bcm_error,  unser_mean, unser_error, bcm_res, gfit_title, gres_title, ped, gain, outfilename, outfile, f_fit, init_par,my_cut,htemp, hsd, max, nbinx, vec_scandata, hsd, bin_content, bin_center,slope;
