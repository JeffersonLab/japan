...
void GetAllPedestals_Caryn_run3430_50uA(int run_num = 3430,TString user_cut ="1",TString scan_data = "bcm_an_ds",double MIN=35,double MAX=65,Int_t myii=0){
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  double gain[nDET];
	...
	sprintf(outfilename,"%s/run%d_detsamcavbcm_pedestal_fit_50uA.txt",
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_det_pedestal_fit_50uA.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
