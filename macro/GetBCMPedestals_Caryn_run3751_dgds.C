...
void GetBCMPedestals_Caryn_run3751_dgds(int run_num = 3751,TString user_cut ="!(scandata1==700&&CodaEventNumber>85800&&CodaEventNumber<89300)&&!(scandata1==550&&CodaEventNumber>121e3&&CodaEventNumber<126e3)&&!(scandata1==300&&CodaEventNumber>162e3)",TString scan_data = "(bcm_dg_ds.hw_sum_raw/bcm_dg_ds.num_samples--396.60)*0.002328",double MIN=5,double MAX=75,Int_t myii=0){
...
//"sam1","sam2","sam3","sam4","sam5","sam6","sam7","sam8",
  ...
  double gain[nDET];
	...
	sprintf(outfilename,"%s/run%d_bcm_pedestal_fit_dgds.txt",
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_bcm_pedestal_fit_dgds_10to70.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
