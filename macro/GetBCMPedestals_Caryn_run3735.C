...
void GetBCMPedestals_Caryn_run3735(int run_num = 3735,TString user_cut ="1",TString scan_data = "unser",double MIN=5,double MAX=75,Int_t myii=0){
...
//"sam1","sam2","sam3","sam4","sam5","sam6","sam7","sam8",
  ...
  double gain[nDET];
	...
	sprintf(outfilename,"%s/run%d_bcm_pedestal_fit_unser.txt",
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_bcm_pedestal_fit_unser.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
