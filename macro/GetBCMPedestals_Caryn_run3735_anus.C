...
void GetBCMPedestals_Caryn_run3735_anus(int run_num = 3735,TString user_cut ="1",TString scan_data = "(bcm_an_us.hw_sum_raw/bcm_an_us.num_samples--669.87)*0.005919",double MIN=5,double MAX=75,Int_t myii=0){
...
//"sam1","sam2","sam3","sam4","sam5","sam6","sam7","sam8",
  ...
  double gain[nDET];
	...
	sprintf(outfilename,"%s/run%d_bcm_pedestal_fit_anus.txt",
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_bcm_pedestal_fit_anus.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
