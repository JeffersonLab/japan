...
void GetBCMPedestals_Caryn_run4229_dgds(int run_num = 4229,TString user_cut ="cleandata && scandata1>3 && !((scandata2==0)&&CodaEventNumber<26e3)&&!(scandata2==2&&unser<10)&&!(scandata2==7&&CodaEventNumber<285e3)",TString scan_data = "(bcm_dg_ds.hw_sum_raw/bcm_dg_ds.num_samples--351.06)*0.002318",double MIN=50,double MAX=75,Int_t myii=0){
...
//"sam1","sam2","sam3","sam4","sam5","sam6","sam7","sam8",
  ...
  gSystem->Exec(Form("convert $(ls -rt %s/run%d_*fit.png) %s/run%d_bcm_pedestal_fit_dgds_55to70.pdf",outputDir.Data(),run_num,outputDir.Data(),run_num));
