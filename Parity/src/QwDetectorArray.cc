...
Int_t QwDetectorArray::ProcessConfigurationBuffer(const UInt_t roc_id, const UInt_t bank_id, UInt_t* buffer, UInt_t num_words)
{
...
Int_t QwDetectorArray::ProcessEvBuffer(const UInt_t roc_id, const UInt_t bank_id, UInt_t* buffer, UInt_t num_words)
{
...
void  QwDetectorArray::ExchangeProcessedData()
{
  ...
  if (1==1 || bNormalization){
    ...
    */
	...
	//QwWarning << "QwDetectorArray::ExchangeProcessedData for loop "<<variable->GetElementName()<< QwLog::endl;
	if (RequestExternalValue(variable->GetElementName(), variable))
	  {
	    if (bDEBUG)
	      dynamic_cast<QwVQWK_Channel*>(variable)->PrintInfo();
	    //QwWarning << "QwDetectorArray::ExchangeProcessedData Found "<<variable->GetElementName()<< QwLog::endl;
	  }
	else
	  {
