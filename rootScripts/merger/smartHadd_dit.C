...
class TreeGetter{
  public:
    ...
    Int_t getData();
    std::vector<std::string> fileglob(const char*);
...
std::vector<std::string> TreeGetter::fileglob(const char *pattern) {
  glob_t g;
...
Int_t TreeGetter::getData() { 
  ...
  for (Int_t i = 0; i < listOfFileIndices.size() ; i++ ){
    ...
    if (((TString)infile2).Contains("#")) {
      Int_t miniruns = 0;
      std::string infile3 = infile2.substr(0,infile2.find("#"));
      std::string infile4 = infile2.substr(infile2.find("#")+1,infile2.size()-infile1.size()-1);
      while (miniruns == 0 || tmpFileInput){
        tmpInfile = Form("%s%d%s%d%s",infile1.c_str(),listOfFileIndices.at(i),infile3.c_str(),miniruns,infile4.c_str());
          for (const auto &tmpfname : fileglob(tmpInfile.c_str())) {
            if( !gSystem->AccessPathName(tmpfname.c_str()) ) {
              Printf("Got datafile %s",tmpfname.c_str());
              tmpFileInput = TFile::Open(tmpfname.c_str());
              infile.push_back(tmpfname);
              fileInput.push_back(tmpFileInput);
              nEntries++;
            }
            else {
              tmpFileInput = NULL;
              //  std::cout << tmpfname << " doesn't exist!!!" << std::endl;
            }
          }
        miniruns++;
      }
...
Int_t TreeGetter::expandData(){
  ...
  //}
  treeList->Add((TChain*)fileInput.at(nEntries-1)->Get(treeName.c_str()));
  for (Int_t f = 0 ; f < nEntries-1; f++) {
    treeList->Add((TChain*)fileInput.at(f)->Get(treeName.c_str()));
  }
...
int smartHadd_dit(std::string listOfFiles = "slug_list.txt", std::string inputFolderName = "/chafs2/work1/apar/aggRootfiles/slugRootfiles/", std::string outputFileName = "grand_aggregator.root", std::string index1 = "n_runs", std::string index2 = "run_number", std::string tree = "agg") {
