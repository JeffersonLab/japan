  ...
  for (Int_t i = 0; i < listOfFileIndices.size() ; i++ ){
    ...
    if (((TString)infile2).Contains("#")) {
      ...
      while (miniruns == 0 || tmpFileInput){
        ...
        if( !gSystem->AccessPathName(tmpInfile.c_str()) ) {
...
int smartHadd_miniruns(std::string listOfFiles = "slug_list.txt", std::string inputFolderName = "/chafs2/work1/apar/aggRootfiles/slugRootfiles/", std::string outputFileName = "grand_aggregator.root", std::string index1 = "n_runs", std::string index2 = "run_number", std::string tree = "agg") {
