...
void TreeGetter::mergeData() {
  ...
  for (int n = 0 ; n < parameterVectors["Trees"].size() ; n++) {
    ...
    //    For each branch that didn't have an entry fill with -1e6 (currently is defaulting to 0)
  ...
  if (worked) {
    if (parameterVectors.count("Prune") != 0 && parameterVectors["Prune"].at(0) == "True"){
...
Int_t TreeGetter::smartAdd(std::string tree) {
  ...
  if ((!fileBaseInput || !fileBaseInput->GetListOfKeys()->Contains(tree.c_str())) && fileInput) {
    Bool_t i1exists = true;
    Bool_t i2exists = true;
...
//    Printf("Scanned temporary copy of baseTree with friends");
    ...
    for(Int_t i=0; i<tmpTree->GetEntries(); i++){
      tmpTree->GetEntry(i);
      if (!i2exists && i >= baseTree->GetEntries()) {
        ind1 = runNumber;
  ...
  if (runNo < 1000 && sluglist != "NULL") { // Assume its a slug - this is for the stripper mostly
    ...
    if (ifinfile.is_open()){
      while(getline(ifinfile,runnum)){
        ...
        getter->runNumber = atoi(runnum.c_str()); //runNo
        if (miniRunNo == 0){
          // FIXME hardcoded to search in the postpan/aggregation minirun stuff for number of miniruns...
          TFile *fin = TFile::Open(Form("/chafs2/work1/apar/postpan-outputs/prexPrompt_%s_000_regress_postpan.root",runnum.c_str()),"READ");
          TTree *T=(TTree*)fin->Get("mini");
          for (Int_t m = 0; m<T->Draw("minirun","","goff"); m++) {
            getter->miniRunNumber = m;
            getter->getData();
          ...
          getter->getData();
          if (getter->parameterVectors.count("Merge") !=0 && getter->parameterVectors["Merge"].at(0) == "True"){
            getter->mergeData();
          }
          else if (getter->parameterVectors.count("Strip") != 0 && getter->parameterVectors["Strip"].at(0) == "True"){
            getter->stripData();
          }
        }
      }
