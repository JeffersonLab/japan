...
void Expand_cyclewise_tagged_dit_to_mini(TString infile, TString outfile, TString treename){
  ...
  out_dit_tree->Branch("minirun_n",&agg_mini);
  ...
  dit_tree->BuildIndex("run","dit_minirun_n");
  ...
  for(int ievt=0;ievt<nEntries_agg;ievt++){
    ...
    Int_t goodbytes = dit_tree -> GetEntry(dit_tree->GetEntryNumberWithIndex((Int_t)agg_run,(Int_t)agg_mini)); // FIXME Add in minirun index for minirun tagged outputs... nested minirun distance loop below here...
    clean_data = 1;
    if (goodbytes==0) {
      clean_data = 0;
      Int_t distance = 0;
      Int_t mini_distance = 1;
      Int_t mini_dist_limit = agg_mini;
      // Look behind us
      while (goodbytes == 0 && distance < 501) {
        mini_distance = 1;
        if (distance == 0) {
          mini_dist_limit = agg_mini;
        }
        else {
          mini_dist_limit = 15;
        }
        // Crawl backwards from the middle of this run
        while (goodbytes == 0 && mini_distance <= mini_dist_limit) {
          goodbytes = dit_tree -> GetEntry(dit_tree->GetEntryNumberWithIndex((Int_t)agg_run-distance,(Int_t)agg_mini-mini_distance));
          mini_distance++;
        }
        distance++;
      }
      ...
      if (distance >= 500) {
        ...
        while (goodbytes == 0 && distance < 501) {
          mini_distance = 1;
          // Crawl forwards from the point in this run
          while (goodbytes == 0 && mini_distance < 15) {
            goodbytes = dit_tree -> GetEntry(dit_tree->GetEntryNumberWithIndex((Int_t)agg_run+distance,(Int_t)agg_mini+mini_distance));
            mini_distance++;
          }
          distance++;
