...
void BMOD::calculateSensitivities(){
  ...
  vector<Double_t> cycleNumbers;
    l_bmwcycnum->GetBranch()->GetEntry(elist->GetEntry(0));
    Double_t cyclenumMin = (Double_t)l_bmwcycnum->GetValue() ;
    l_bmwcycnum->GetBranch()->GetEntry(elist->GetEntry(nonzero-1));
    Double_t cyclenumMax = (Double_t)l_bmwcycnum->GetValue() ;
  for(int i=0;i<1001;i++){
    //l_bmwcycnum->GetBranch()->GetEntry(elist->GetEntry(i));
    if (cyclenumMax==cyclenumMin) cyclenumMax = cyclenumMin+1.0;
    Double_t cyclenum = cyclenumMin + 1.0*i/1000.0 * (cyclenumMax-cyclenumMin) ; // (Double_t)l_bmwcycnum->GetValue() ;
    //if(i==0){
    ...
    while(j<(nBPM+nDet) && cycles[i].BPMvalid!=0){
      ...
      for(int icoil=0;icoil<nCoil;icoil++){
        ...
        if (i == 0) {
          ...
          if(ndata<50){
            ...
            if (j<nBPM) {
              ...
              j=nBPM+nDet;
              continue;
              // FIXME Only loop through these on the first pass. Otherwise just read the 0th pass/return 0 when this is bad.
            }
            else if (j>=nBPM) {
              cycles[i].Detvalid = 0;
              icoil = nCoil;
        ...
        //else if(ndata>=50 && cycles[i].BPMvalid==1)
          ...
          if (i==0) {
            int ndata2 = tree_R->Draw(Form("(%lf/%lf)*(%s):(bmod_trim%s*%lf)",factor,this_mean,name.Data(),parameterVectors["Coils"][icoil].c_str(),chtov),
            Form("%s && beam_mod_ramp>0 && bmwobj==%s && abs(bmod_trim%s-%f)>20",
              //Form("%s && bmod_ramp>0 && bmwobj==%s && abs(bmod_trim%s-%f)>20 && bmwcycnum==%f",
            parameterVectors["Cut"].at(0).c_str(),parameterVectors["Coils"][icoil].c_str(),parameterVectors["Coils"][icoil].c_str(),trim_base[icoil]));
            ...
            Printf("-- CycleNumber: %f",cycles[i].cycleNumber);
            Printf("-- Coil ID: %d",icoil+1);
            Printf("-- NData: %d",ndata2);

            TGraph* g2 = new TGraph(ndata2, tree_R->GetV2(),tree_R->GetV1());
            fun->SetParameters(init_par);
            g2->Fit("fun","Q0");
            ...
            if (j<nBPM) {
              cycles.at(i).BPMfNdata.at(j).at(icoil) += ndata2;
              cycles.at(i).BPMsens_rand[j][icoil]     = rand->Gaus(0,err);
              cycles.at(i).BPMsens[j][icoil]          = this_slope + cycles.at(i).BPMsens_rand[j][icoil]*this_error;
              cycles.at(i).BPMsens_err[j][icoil]      = this_error;
            }
            else {
              cycles.at(i).DetfNdata[j-nBPM][icoil]  += ndata2;
              cycles.at(i).Detsens_rand[j-nBPM][icoil]= rand->Gaus(0,err);
              cycles.at(i).Detsens[j-nBPM][icoil]     = this_slope + cycles.at(i).Detsens_rand[j-nBPM][icoil]*this_error;
              cycles.at(i).Detsens_err[j-nBPM][icoil] = this_error;
            }
          }
          else { 
            if (j<nBPM) {
              cycles.at(i).BPMfNdata.at(j).at(icoil) += cycles.at(0).BPMfNdata.at(j).at(icoil);
              cycles.at(i).BPMsens_rand[j][icoil]     = rand->Gaus(0,err);
              cycles.at(i).BPMsens[j][icoil]          = cycles.at(0).BPMsens[j][icoil] + cycles.at(i).BPMsens_rand[j][icoil]*cycles.at(0).BPMsens_err[j][icoil];
              cycles.at(i).BPMsens_err[j][icoil]      = cycles.at(0).BPMsens_err[j][icoil];
            }
            else {
              cycles.at(i).DetfNdata[j-nBPM][icoil]  += cycles.at(0).DetfNdata.at(j-nBPM).at(icoil);
              cycles.at(i).Detsens_rand[j-nBPM][icoil]= rand->Gaus(0,err);
              cycles.at(i).Detsens[j-nBPM][icoil]     = cycles.at(0).Detsens[j-nBPM][icoil] + cycles.at(i).Detsens_rand[j-nBPM][icoil]*cycles.at(0).Detsens_err[j-nBPM][icoil];
              cycles.at(i).Detsens_err[j-nBPM][icoil] = cycles.at(0).Detsens_err[j-nBPM][icoil];
            }
          }
        }
      } // end of coil loop
...
int runRandBmodAna(Int_t runNo = 4199, Double_t error = 1.0, std::string inputFile = "input.txt", TString sluglist = "NULL"){
