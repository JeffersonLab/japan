...
void plotCompare5bpmsPlainDitRegSlopes(TString suffix = ""){ //TString input = "NULL",TString runNum = "", TString slugNum = "0"){}
  TString old_suffix = "";
  std::map<int,Double_t> runs;
  // Grab all of the relevant combos
  TChain* mini = new TChain("mini");
  TChain* mini_reg_5bpms = new TChain(Form("agg_plain_friendable"));         // reg_asym_usl_diff_bpm4eX_slope
  TChain* mini_dit_5bpms = new TChain(Form("dit_plain_run_avg_friendable")); // usl_12X_mean
  mini                                    ->  AddFile(Form("../respin2_data/eigen_noerr_part_avg_allbpms_lagr_slopes.root"));
  mini_reg_5bpms ->  AddFile(Form("../processed_respin2_data/mini_friended_agg.root"));
  mini_dit_5bpms ->  AddFile(Form("../processed_respin2_data/All_dit_outputs_doing_segment_averaging_of_slopes_mini_friended.root"));
  ...
  std::vector<TString> devices5_reg = {
    "diff_bpm1X_slope",
    "diff_bpm4aY_slope",
    "diff_bpm4eX_slope",
    "diff_bpm4eY_slope",
  ...
  for (Int_t i = 0 ; i < devices5.size() ; i++) {
    mini_reg_5bpms->SetAlias(Form("usl_%s",devices5.at(i).Data()),Form("1.0e3*reg_asym_usl_diff_bpm%s_slope",devices5.at(i).Data()));
    mini_reg_5bpms->SetAlias(Form("usr_%s",devices5.at(i).Data()),Form("1.0e3*reg_asym_usr_diff_bpm%s_slope",devices5.at(i).Data()));
    mini_dit_5bpms->SetAlias(Form("usl_%s",devices5.at(i).Data()),Form("usl_%s_mean",devices5.at(i).Data()));
    mini_dit_5bpms->SetAlias(Form("usr_%s",devices5.at(i).Data()),Form("usr_%s_mean",devices5.at(i).Data()));
  }

  mini->AddFriend(mini_reg_5bpms,"slopes");
  mini->AddFriend(mini_dit_5bpms,"dit");
  ...
  c1->SaveAs(Form("october_25_plots/12BPM_evMon_segmented_eigen_reg_slopes_compare%s.pdf",suffix.Data()));
  ...
  c1_left->SaveAs(Form("october_25_plots/12BPM_evMon_segmented_eigen_reg_slopes_compare_left%s.pdf",suffix.Data()));
  ...
  c1_right->SaveAs(Form("october_25_plots/12BPM_evMon_segmented_eigen_reg_slopes_compare_right%s.pdf",suffix.Data()));
  */
  ...
  //c2->SetTitle(ana);
  //c2->SetName(ana);
  c2->Divide(2,devices5.size());
  //cut = "run>6332 && run<7500";
  for (Int_t j = 0 ; j < devices5.size() ; j++) {
    cut_reg = "rcdb_run_type==1 && rcdb_run_flag==1 && rcdb_arm_flag!=1";
    cut_dit = "rcdb_run_type==1 && rcdb_run_flag==1 && rcdb_arm_flag!=1";
    p1=combo_segment_compare_multiGraph(c2,p1,mini,0+2*j+1,Form("slopes.usl"),Form("dit.usl"),cut_reg,cut_dit,"dit_segment",devices5.at(j),36+2*j);
    cut_reg = "rcdb_run_type==1 && rcdb_run_flag==1 && rcdb_arm_flag!=2";
    cut_dit = "rcdb_run_type==1 && rcdb_run_flag==1 && rcdb_arm_flag!=2";
    p1=combo_segment_compare_multiGraph(c2,p1,mini,0+2*j+2,Form("slopes.usr"),Form("dit.usr"),cut_reg,cut_dit,"dit_segment",devices5.at(j),36+2*j);
  }
  ...
  label->SetText(0.0,0.005,ana);
  label->Draw("same");
  //c2->SaveAs(Form("october_25_plots/%s(",pdfname.Data()));
  c2->SaveAs(Form("october_25_plots/12BPM_evMon_segmented_eigen_reg_slopes_compare_segs%s.pdf",suffix.Data()));
  ...
  for (Int_t j = 0 ; j < devices5.size() ; j++) {
    cut_reg = "rcdb_run_type==1 && rcdb_run_flag==1 && rcdb_arm_flag!=1";
    cut_dit = "rcdb_run_type==1 && rcdb_run_flag==1 && rcdb_arm_flag!=1";
    p1=combo_segment_compare_multiGraph(c3,p1,mini,0+2*j+1,Form("1e3*mini_eigen_reg_allbpms_part_avg.usl"),Form("1e3*mini_eigen_lagr_allbpms_part_avg.usl"),cut_reg,cut_dit,"dit_segment",devices5.at(j),30+2*j);
    cut_reg = "rcdb_run_type==1 && rcdb_run_flag==1 && rcdb_arm_flag!=2";
    cut_dit = "rcdb_run_type==1 && rcdb_run_flag==1 && rcdb_arm_flag!=2";
    p1=combo_segment_compare_multiGraph(c3,p1,mini,0+2*j+2,Form("1e3*mini_eigen_reg_allbpms_part_avg.usr"),Form("1e3*mini_eigen_lagr_allbpms_part_avg.usr"),cut_reg,cut_dit,"dit_segment",devices5.at(j),30+2*j);
  }
  ...
  label->SetText(0.0,0.005,ana);
  label->Draw("same");
  c3->SaveAs(Form("october_25_plots/%s)",pdfname.Data()));
  c3->SaveAs(Form("october_25_plots/12BPM_evMon_segmented_eigen_reg_slopes_compare_segs%s.pdf)",suffix.Data()));
  */
