  ...
  */  
  ...
  for(Int_t i=0;i<cycles.size();i++){
    ...
    while(j<(nBPM+nDet) && cycles[i].BPMvalid!=0){
      ...
      if (parameterVectors.count("Coils") == 0 || (parameterVectors.count("Coils") != 0 && parameterVectors["Coils"].size() == 0)) {
      ...
      for(int icoil=0;icoil<nCoil;icoil++){
        ...
        if (ndata<ndat && ((parameterVectors.count("Stitch") == 0 || (parameterVectors.count("Stitch") != 0 && parameterVectors["Stitch"].at(0) != "True")) || i == 0)) {
          ...
          // Else we want to stitch neighboring runs together
          // Ignore the first cycle (since it can't look back farther)
          for(Int_t s = i-1; s>=0; s--) {
            if (s < i-2) { // Then give up on that run
              if (j<nBPM) {
                if (cycles.at(s).BPMfNdata.at(j).at(icoil)<ndat) { // Then give up on that run
                  cycles.at(s).BPMvalid = 0;
                }
                s = 0; 
                continue;
              } // Break out if 20 minutes away
              else {
                if (cycles.at(s).DetfNdata.at(j-nBPM).at(icoil)<ndat) { // Then give up on that run
            ...
            if (j<nBPM) {
              if(cycles.at(s).BPMfNdata.at(j).at(icoil)>=ndat && cycles.at(i).BPMfNdata.at(j).at(icoil)<ndat) {
                Printf("-- Sensitivity for Device: %s",name.Data());
                Printf("-- CycleNumber: %f",cycles[i].cycleNumber);
                Printf("-- Coil ID: %d",icoil+1);
                Printf("-- Taking data from prior cycle: %f",cycles.at(s).cycleNumber);
                Printf("-- NData from this cycle: %d",ndata);
                Printf("-- NData from prior cycle: %f",cycles.at(s).BPMfNdata.at(j).at(icoil));
                Printf("-- Cycle's Stitch Counter: %d",cycles.at(i).sensStitchedCounter);
                cycles.at(i).BPMfNdata.at(j).at(icoil) += cycles.at(s).BPMfNdata.at(j).at(icoil);
                cycles.at(i).BPMsens[j][icoil]          = cycles.at(s).BPMsens[j][icoil];
                cycles.at(i).BPMsens_stitch[j][icoil]   = cycles.at(s).cycleNumber;
                cycles.at(i).BPMsens_err[j][icoil]      = cycles.at(s).BPMsens_err[j][icoil];
                cycles.at(i).sensStitched               = 1;
                cycles.at(i).sensStitchedCounter       += 1;
                if (parameterVectors.count("Stitch Limit") != 0 && parameterVectors["Stitch Limit"].size() > 0 && cycles.at(i).sensStitchedCounter > std::atoi(parameterVectors["Stitch Limit"].at(0).c_str())) {
                  // Don't do too many gap-fills
                  Printf("-- Too many stitches, abandoning");
                  cycles.at(i).BPMvalid = 0;
                }
                else {
                  Printf("-- ");
                ...
                Printf("-- ");
                if (j<nBPM) {
                  if (parameterVectors.count("Stitch") == 0 || (parameterVectors.count("Stitch") != 0 && parameterVectors["Stitch"].at(0) != "True")) {
                    cycles[i].BPMvalid = 0;
                    Printf("-- Not enough data Discarding this cycle ");
                  }
                  else {
              ...
              if(cycles.at(s).DetfNdata.at(j-nBPM).at(icoil)>=ndat && cycles.at(i).DetfNdata.at(j-nBPM).at(icoil)<ndat) {
                Printf("-- Sensitivity for Device: %s",name.Data());
                Printf("-- CycleNumber: %f",cycles[i].cycleNumber);
                Printf("-- Coil ID: %d",icoil+1);
                Printf("-- Taking data from prior cycle: %f",cycles.at(s).cycleNumber);
                Printf("-- NData from this cycle: %d",ndata);
                Printf("-- NData from prior cycle: %f",cycles.at(s).DetfNdata.at(j-nBPM).at(icoil));
                Printf("-- Cycle's Stitch Counter: %d",cycles.at(i).sensStitchedCounter);
                cycles.at(i).DetfNdata[j-nBPM][icoil]     += cycles.at(s).DetfNdata.at(j-nBPM).at(icoil);
                cycles.at(i).Detsens[j-nBPM][icoil]        = cycles.at(s).Detsens[j-nBPM][icoil];
                cycles.at(i).Detsens_stitch[j-nBPM][icoil] = cycles.at(s).cycleNumber;
                cycles.at(i).Detsens_err[j-nBPM][icoil]    = cycles.at(s).Detsens_err[j-nBPM][icoil];
                cycles.at(i).sensStitched                  = 1;
                if (parameterVectors.count("Important Detectors") == 0 || (parameterVectors.count("Important Detectors") != 0 && parameterVectors["Important Detectors"].size() > 0 && (j-nBPM) < std::atoi(parameterVectors["Important Detectors"].at(0).c_str()))) {
                  cycles.at(i).sensStitchedCounter        += 1;
                }
        ...
        else if(ndata>=ndat && cycles[i].BPMvalid==1){
          ...
          if (j<nBPM) {
            ...
            for (Int_t s = i-1; s>=0 ; s--) {
              if (cycles.at(s).BPMvalid != 0) {
                ...
                if(cycles.at(s).BPMfNdata.at(j).at(icoil)<ndat) { // Need to fill in the data
                  Printf("-- Giving data to prior cycle: %f",cycles.at(s).cycleNumber);
                  cycles.at(s).BPMfNdata.at(j).at(icoil) += cycles.at(i).BPMfNdata.at(j).at(icoil);
                  cycles.at(s).BPMsens[j][icoil]          = cycles.at(i).BPMsens[j][icoil];
                  cycles.at(s).BPMsens_stitch[j][icoil]   = cycles.at(i).cycleNumber;
                  cycles.at(s).BPMsens_err[j][icoil]      = cycles.at(i).BPMsens_err[j][icoil];
                  cycles.at(s).sensStitched               = 1;
                  cycles.at(s).sensStitchedCounter       += 1;
                  if (parameterVectors.count("Stitch Limit") != 0 && parameterVectors["Stitch Limit"].size() > 0 && cycles.at(s).sensStitchedCounter > std::atoi(parameterVectors["Stitch Limit"].at(0).c_str())) {
                    // Don't do too many gap-fills
                    Printf("-- Too many stitches, abandoning");
                    cycles.at(s).BPMvalid = 0;
                  }
                  ...
                  Printf("-- Prior cycle needs no more data");
                  Printf("-- ");
                  Printf("-- ");
                  Printf("-- ");
                }
            ...
            for (Int_t s = i-1; s>=0 ; s--) {
              if (cycles.at(s).BPMvalid != 0) {
                ...
                if(cycles.at(s).DetfNdata.at(j-nBPM).at(icoil)<ndat) { // Need to fill in the data
                  Printf("-- Giving data to prior cycle: %f",cycles.at(s).cycleNumber);
                  cycles.at(s).DetfNdata.at(j-nBPM).at(icoil) += cycles.at(i).DetfNdata.at(j-nBPM).at(icoil);
                  cycles.at(s).Detsens[j-nBPM][icoil]          = cycles.at(i).Detsens[j-nBPM][icoil];
                  cycles.at(s).Detsens_stitch[j-nBPM][icoil]   = cycles.at(i).cycleNumber;
                  cycles.at(s).Detsens_err[j-nBPM][icoil]      = cycles.at(i).Detsens_err[j-nBPM][icoil];
                  cycles.at(s).sensStitched                    = 1;
                  if (parameterVectors.count("Important Detectors") == 0 || (parameterVectors.count("Important Detectors") != 0 && parameterVectors["Important Detectors"].size() > 0 && (j-nBPM) < std::atoi(parameterVectors["Important Detectors"].at(0).c_str()))) {
                    cycles.at(s).sensStitchedCounter          += 1;
                  }
...
int bmodStitch(Int_t runNo = 4199, std::string inputFile = "input.txt", TString sluglist = "NULL"){
