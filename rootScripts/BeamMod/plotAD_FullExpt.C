...
void plotAD_FullExpt(std::string input = "NULL",std::string runNum = "", std::string slugNum = "0"){
  ...
  std::string pdfname = Form("%s%s_%salphas-deltas_FullExpt.pdf",input.substr(0,input.find(".root")).c_str(),slugNum.c_str(),runNum.c_str());

  if (((TString)pdfname).Contains("run")) {
    pdfname = Form("%s%s_alphas-deltas_FullExpt.pdf",input.substr(0,input.find(".root")).c_str(),slugNum.c_str());
  }
