...
void PlotInjBPMS_FC2_OPS_5min(TString ucut){

  vector<const char*> vBPMops5 = {"bpm2i01","bpm2i02","bpm1i02","bpm1i04",
                               "bpm1i06", "bpm0i01","bpm0i01a","bpm0i02",
                               "bpm0i02a","bpm0i05","bpm0i07","bpm0l01",
                               "bpm0l02"};
  ...
  for(Int_t dd=0;dd<ndeti;dd++) {
    mul_tree->Draw(Form("1e6*asym_%sWS>>hista%d",vBPMops5[dd],dd),lcut,"goff");
    ...
    // delete thishist;
    mul_tree->Draw(Form("1e3*diff_%sY>>histy%d",vBPMops5[dd],dd),lcut,"goff");
    ...
    // delete thishist;
    // cout<<vBPMops5[dd]<<" Aq[ppm](RMS)+-error,Dx[nm](RMS)+-err,Dy[nm](RMS)+-err: "<<Aq[dd]<<" "<<rmsAq[dd]<<" "<<eAq[dd]<<" "<<1e3*Dx[dd]<<" "<<1e3*rmsDx[dd]<<" "<<1e3*eDx[dd]<<" "<<1e3*Dy[dd]<<" "<<1e3*rmsDy[dd]<<" "<<1e3*eDy[dd]<<endl;
  ...
  for(Int_t k=0;k<numchains;k++){
    xygraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vBPMops5[k]);
  }
  ...
  for(Int_t k=0;k<numchains;k++){
    xyrmsgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vBPMops5[k]);
  }
  ...
  for(Int_t k=0;k<numchains;k++){
    Aqgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vBPMops5[k]);
  }
  ...
  for(Int_t k=0;k<numchains;k++){
    Aqrmsgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vBPMops5[k]);
  }
