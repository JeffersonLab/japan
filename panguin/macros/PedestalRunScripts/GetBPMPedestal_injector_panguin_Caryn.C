...
void GetBPMPedestal_injector_panguin_Caryn(TString devnam= "1i02", TString scan_data = "scandata1",TString user_cut ="1",Double_t MIN=-1e6, Double_t MAX=1e6,Int_t myii=0){
  // gROOT->Reset();
...
const int nBPM= sizeof(device_name)/sizeof(*device_name);
  ...
  for(int ibpm=0;ibpm<nBPM;ibpm++){
	...
	mg_fit[ich]->Add(g_fit_ref[ich]);
      mg_fit[ich]->Draw("AP");
      gfit_title[ich] = Form("bpm%s%s;%s; Raw ADC",device_name[ibpm].Data(),ch_name[ich].Data(),scan_data.Data());
	...
	mg_res[ich]->Add(g_res_ref[ich]); 
      mg_res[ich]->Draw("AP");
      gres_title[ich] = Form("bpm%s%s;%s; Residual ",device_name[ibpm].Data(),ch_name[ich].Data(),scan_data.Data());
  ...
  // delete htemp,hsd,h_stat,ped,slope;
  //  delete g_res,g_fit, g_res_ref,g_fit_ref,mg_res,mg_fit;

    delete hsd;
    delete tree,htemp,adc_mean,adc_error,adc_res,h_stat,ped,slope,device_name,max,nbinx,vec_scandata,bin_content, bin_center,ndata,ndata_fit,scandata,scandata_error,startpt,nBPM,ch_name,f_zero,my_cut,branch_name,num_samples_name,gfit_title,gres_title, f_fit, init_par;
    //   delete c_fit;
   //   gDirectory->Delete(g_res);
   ///   g_res->SetBit(kCandelete);
  //SafeDelete(g_res);
}
