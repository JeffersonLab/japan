...
void GetMainDetPedestalR_panguin_Caryn(TString user_cut ="1",TString scan_data = "unser",double MIN=-1000,double MAX=1000,Int_t myii=0){
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  TPad *c_detl = new TPad("c_detl","c_detl",0,0,1,1);
  c_detl->Divide(1,2);
  c_detl->Draw();
  TVirtualPad* cl_fit = c_detl->cd(1);
  TVirtualPad* cl_res = c_detl->cd(2);
  cl_fit->Divide(nDET,1);
  cl_res->Divide(nDET,1);
  ...
  for(int idet=0;idet<nDET;idet++){
	...
	det_error[i] = h_stat->GetRMS()/TMath::Sqrt(h_stat->GetEntries());
      ...
      g_detl_fit = new TGraphErrors(ndata,unser_mean,det_mean,unser_error,det_error);
      g_detl_fit->SetMarkerStyle(20);
      g_detl_fit->Draw("AP");
      gfit_title = Form("%s; %s; DET(ADC)",device_name[idet].Data(),scan_data.Data());
      g_detl_fit->SetTitle(gfit_title.Data());
      f_fit->SetParameters(init_par);
      f_fit->SetLineColor(kRed);
      g_detl_fit->Fit("f_fit","QR","",MIN,MAX);
