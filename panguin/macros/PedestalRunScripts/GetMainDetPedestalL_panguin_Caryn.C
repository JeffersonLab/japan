...
void GetMainDetPedestalL_panguin_Caryn(TString user_cut ="1",TString scan_data = "unser",double MIN=-1000,double MAX=1000,Int_t myii=0){
 ...
 tree->SetAlias("beam_current", scan_data);
  ...
  for(int idet=0;idet<nDET;idet++){
	...
	det_error[i] = h_stat->GetRMS()/TMath::Sqrt(h_stat->GetEntries());
      ...
      g_detl_fit->Fit("f_fit","Q","",MIN,MAX);
