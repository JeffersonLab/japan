...
void PlotInjBPMS_FC2_OPS(TString ucut){
  ...
  for(Int_t dd=0;dd<ndeti;dd++) {
    mul_tree->Draw(Form("1e6*asym_%sWS>>hista%d",vBPM[dd],dd),lcut,"goff");
    ...
    zero[dd]=0;
    // delete thishist;
    mul_tree->Draw(Form("1e3*diff_%sX>>histx%d",vBPM[dd],dd),lcut,"goff");
    ...
    // delete thishist;
    mul_tree->Draw(Form("1e3*diff_%sY>>histy%d",vBPM[dd],dd),lcut,"goff");
