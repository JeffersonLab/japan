// A macro for checking Sync offline - on old or live runs, just check stdout
#include "/adaqfs/home/apar/PREX/japan/rootScripts/Open.C"
void SyncCheckManually(Int_t runNum = 999999){
  ...
  if (!r) {
    ...
    gSystem->Exec("echo \"OK\"");
    return;
  }
  else {
    TH1D* hist = new TH1D("CodaEventNumber", "CodaEventNumber", 100, 0, 10e9);

    r->Project(hist->GetName(), "CodaEventNumber", "1==1");
    ...
    if (sum > 8) {
      alarmString = Form("SYNC LOST: %d events = %.2f%% lost",sum,persum);
    }
    else {
      alarmString = "OK";
    }
    ...
    gSystem->Exec(Form("echo \"%s\" > /adaqfs/home/apar/scripts/stats/sync_%d.dat",string.Data(),runNum));
    if(hist) delete hist;
  }
}
