...
vector<const char*> vBPMHallshort = {"bpm8","bpm12","bpm14","bpm1p02b","bpm1p03a","bpm1","bpm4a","bpm4ac","bpm4e","bpm4ec"};

void PlotHallBPMSshort(TString ucut){
  ...
  Int_t ndeti = vBPMHallshort.size();
  ...
  for(Int_t dd=0;dd<ndeti;dd++) {
    mul_tree->Draw(Form("1e6*asym_%sWS>>hista%d",vBPMHallshort[dd],dd),lcut,"goff");
    ...
    zero[dd]=0;
    // delete thishist;
    mul_tree->Draw(Form("1e3*diff_%sX>>histx%d",vBPMHallshort[dd],dd),lcut,"goff");
    ...
    // delete thishist;
    mul_tree->Draw(Form("1e3*diff_%sY>>histy%d",vBPMHallshort[dd],dd),lcut,"goff");
    ...
    // delete thishist;
    // cout<<vBPMHallshort[dd]<<" Aq[ppm](RMS)+-error,Dx[nm](RMS)+-err,Dy[nm](RMS)+-err: "<<Aq[dd]<<" "<<rmsAq[dd]<<" "<<eAq[dd]<<" "<<1e3*Dx[dd]<<" "<<1e3*rmsDx[dd]<<" "<<1e3*eDx[dd]<<" "<<1e3*Dy[dd]<<" "<<1e3*rmsDy[dd]<<" "<<1e3*eDy[dd]<<endl;
  ...
  for(Int_t k=0;k<numchains;k++){
    xygraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vBPMHallshort[k]);
  }
  ...
  for(Int_t k=0;k<numchains;k++){
    xyrmsgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vBPMHallshort[k]);
  }
  ...
  for(Int_t k=0;k<numchains;k++){
    Aqgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vBPMHallshort[k]);
  }
  ...
  for(Int_t k=0;k<numchains;k++){
    Aqrmsgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vBPMHallshort[k]);
  }
