...
//vector<const char*> vBPMHall = {"bpm8","bpm12","bpm14","bpm1p02b","bpm1p03a","bpm1","bpm4a","bpm4ac","bpm4e","bpm4ec"};
vector<const char*> vBPMHall = {"bpm12","bpm4a","bpm4e", "T", "Tp", "E"};

void PlotHallBPMSprojected(TString ucut){
  ...
  mul_tree -> SetAlias("Tp_y","diff_bpm4eY-diff_bpm4aY/4.083");
  ...
  for(Int_t dd=0;dd<ndeti;dd++) {    
    ...
    // delete thishist;
    mul_tree->Draw(Form("1e3*diff_%sY>>histy%d",vBPMHall[dd],dd),lcut,"goff");
    ...
    // delete thishist;
    // cout<<vBPMHall[dd]<<" Aq[ppm](RMS)+-error,Dx[nm](RMS)+-err,Dy[nm](RMS)+-err: "<<Aq[dd]<<" "<<rmsAq[dd]<<" "<<eAq[dd]<<" "<<1e3*Dx[dd]<<" "<<1e3*rmsDx[dd]<<" "<<1e3*eDx[dd]<<" "<<1e3*Dy[dd]<<" "<<1e3*rmsDy[dd]<<" "<<1e3*eDy[dd]<<endl;
    }

    //These are the Xt and Yt (dd=3) and Xt' and Yt' (dd=4) variables
    else if(dd==3||dd==4){
    Aq[dd] = 0;
    eAq[dd]= 0;
    rmsAq[dd]= 0;    
    ...
    mul_tree->Draw(Form("1e3*%s_y>>histy%d",vBPMHall[dd],dd),lcut,"goff");
    objnam = Form("histy%d",dd);
    thishist = (TH1D*)gDirectory->FindObject(objnam);
    Dy[dd] = thishist->GetMean();
    eDy[dd]= thishist->GetMeanError();
  ...
  for(Int_t k=0;k<numchains;k++){
    xygraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vBPMHall[k]);
  }
  ...
  for(Int_t k=0;k<numchains;k++){
    xyrmsgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vBPMHall[k]);
  }
  ...
  for(Int_t k=0;k<numchains;k++){
    Aqgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vBPMHall[k]);
  }
  ...
  for(Int_t k=0;k<numchains;k++){
    Aqrmsgraph->GetHistogram()->GetXaxis()->SetBinLabel(k+1,vBPMHall[k]);
  }
