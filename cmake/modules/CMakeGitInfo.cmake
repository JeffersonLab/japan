# Source: https://github.com/andrew-hardin/cmake-git-version-tracking
...
function(GitStateChangedAction _state_as_list)
    ...
    LIST(GET _state_as_list 3 GIT_STATUS)
    LIST(GET _state_as_list 4 GIT_LOG)
    LIST(GET _state_as_list 5 GEANT_VERSION)
    LIST(GET _state_as_list 6 ROOT_VERSION)
...
function(GetGitState _working_dir _state)
    ...
    execute_process(COMMAND
        "${GIT_EXECUTABLE}" status --branch --short --porcelain
        WORKING_DIRECTORY "${_working_dir}"
        RESULT_VARIABLE res
    ...
    # Get last commit log message
    execute_process(COMMAND
        "${GIT_EXECUTABLE}" --no-pager log -n 1
        WORKING_DIRECTORY "${_working_dir}"
        RESULT_VARIABLE res
        OUTPUT_VARIABLE _git_log
        ERROR_QUIET
        OUTPUT_STRIP_TRAILING_WHITESPACE)
    if(NOT res EQUAL 0)
        set(_success "false")
        set(_git_log "GIT-NOTFOUND")
    endif()

    # Get geant4 version string
    execute_process(COMMAND
        geant4-config --version
        WORKING_DIRECTORY "${_working_dir}"
        RESULT_VARIABLE res
        OUTPUT_VARIABLE _geant_version
        ERROR_QUIET
        OUTPUT_STRIP_TRAILING_WHITESPACE)
    if(NOT res EQUAL 0)
        set(_success "false")
        set(_geant_version "GEANT-CONFIG-NOTFOUND")
    endif()
    ...
    if(NOT res EQUAL 0)
        set(_success "false")
        set(_root_version "ROOT-CONFIG-NOTFOUND")
    endif()

    # Return a list of our variables to the parent scope.
    set(${_state} ${_success} ${_hashvar} ${_dirty} ${_git_status} ${_git_log} ${_geant_version} ${_root_version} PARENT_SCOPE)
endfunction()
